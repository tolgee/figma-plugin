...
<header
  class="sticky top-0 z-10 flex items-center gap-2 border-b border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2"
>
  <span
    class="text-sm font-bold tracking-tight text-[var(--color-text-brand)]"
  >
    Tolgee
  </span>
  ...
  {#if auth.value.authenticated}
    <div class="flex flex-1 items-center gap-2">
      {#if languageOptions.length > 0}
        ...
        />
      {:else}
        <span class="text-xs text-[var(--color-text-secondary)] italic">
