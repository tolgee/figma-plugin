...
import { SetupHandle, TolgeeConfig } from "../../types";
...
export const Settings = () => {
  ...
  const validateTolgeeCredentials = async () => {
    ...
    try {
      const res = await sendTolgeeRequest(
        "v2/api-keys/current",
        "GET",
        tolgeeConfig as TolgeeConfig
      );
      if (
        res &&
        res.status === 200 &&
        res.data?.scopes?.includes("translations.view") &&
        res.data?.scopes?.includes("translations.view")
      ) {
        getTolgeeLanguages();
        setOpen(false);
  ...
  const getTolgeeLanguages = async () => {
    try {
      ...
      if (res && res.status === 200 && res.data?._embedded?.languages?.length) {
        setLanguages(
          res.data._embedded.languages.map((e: any) => ({
            value: e.tag,
            text: e.name,
          }))
        );
        setTolgeeConfig({
          ...tolgeeConfig,
          lang: tolgeeConfig.lang ?? res.data._embedded.languages[0].tag,
        });
      } else {
  ...
  const handleSubmit = () => {
    ...
    if (
      tolgeeConfig.apiKey !== previousConfig.apiKey ||
      tolgeeConfig.url !== previousConfig.url
    ) {
      validateTolgeeCredentials();
      return;
    }
    ...
    <Container space="medium">
      ...
      <Dropdown
        ...
        onChange={({ currentTarget: { value: lang } }) =>
          setTolgeeConfig({ ...tolgeeConfig, lang })
        }
        options={languages}
        value={
          languages.find((e: any) => e.value === tolgeeConfig.lang)
            ? tolgeeConfig.lang ?? null
            : null
        }
      />
