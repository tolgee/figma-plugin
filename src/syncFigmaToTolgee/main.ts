...
export default async function () {
  ...
  (
    ...
    .forEach((node) => {
      ...
      if (
        nodes.findIndex(
          (n) => n.characters === node.characters && node.name === n.name
        ) === -1
      ) {
        nodes.push(node);
      }
  ...
  nodes = nodes.sort((n1, n2) => {
    if (
      conflictingNodes.includes(n1) &&
      conflictingNodes.includes(n2) &&
      n1.name === n2.name
    ) {
      return -1;
    } else {
  ...
  if (config && config.lang && config.apiKey && config.apiUrl) {
    showUI(
      {
        height: 800,
        width: 500,
