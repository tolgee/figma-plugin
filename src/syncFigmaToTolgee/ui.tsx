...
function Plugin({
  ...
  const [tolgeeData, setTolgeeData] = useState<Record<string, any> | null>(
    null
  );
  ...
  useEffect(() => {
    setLoading(true);
    sendTolgeeRequest(
      `v2/projects/translations/${selectedLanguage}`,
      "GET",
      config as TolgeeConfig
    ).then((res) => {
      ...
      if (tolgeeData) {
        setNewNodes(
          nodes.filter(
            (node) =>
              !Object.keys(tolgeeData).includes(
                node.name.slice(TOLGEE_PREFIX.length)
              ) && !conflictingNodes.find((n) => n.name === node.name)
          )
        );
        setDuplicateNodes(
          nodes.filter(
            (node) => !!conflictingNodes.find((n) => n.name === node.name)
          )
        );
        setEditedNodes(
          nodes.filter(
            (node) =>
              ...
              tolgeeData[node.name.slice(TOLGEE_PREFIX.length)] !==
                node.characters &&
              !conflictingNodes.find((n) => n.name === node.name)
          )
        );
  ...
  useEffect(() => {
    ...
    sendTolgeeRequest(
      ...
      if (res && res.status === 200 && res.data?._embedded?.languages?.length) {
        setLoading(false);
        setLanguages(
          res.data._embedded.languages.map((l: any) => ({
            value: l.tag,
            text: l.name,
          }))
        );
  ...
  useEffect(() => {
    ...
    if (selectAll) {
      setSelectedNodes(
        nodes.filter(
          (node) =>
            !conflictingNodes.find(
              (n) => n.name === node.name && n.characters !== node.characters
            )
        )
      );
    } else {
  ...
  const toggleNode = (node: NodeInfo) => {
    if (selectedNodes.includes(node)) {
      ...
      if (selectedNodes.find((n) => n.name === node.name)) {
        setSelectedNodes([
          ...selectedNodes.filter((n) => n.name !== node.name),
          node,
        ]);
      } else {
  ...
  const syncKeys = () => {
    ...
    selectedNodes.forEach((node) => {
      promises.push(
        sendTolgeeRequest(
          ...
          config,
          JSON.stringify({
            key: node.name.slice(TOLGEE_PREFIX.length),
            translations: { [config.lang]: node.characters },
            languagesToReturn: [config.lang],
          })
        )
      );
    });
    Promise.allSettled(promises).then((results) => {
      const successfulResults = results
        ...
        .map((res) => res.status === "fulfilled" && res.value?.data)
        .map((d) =>
          d ? { key: d.keyName, value: d.translations[config.lang].text } : null
        )
        .filter((r) => !!r?.value);
      const affectedDuplicatedNodes = [];
      for (const result of successfulResults) {
        setSelectedNodes([]);
        affectedDuplicatedNodes.push(
          ...duplicateNodes
            .filter((n) => n.name.slice(TOLGEE_PREFIX.length) === result!.key)
            .map((n) => ({ ...n, value: result!.value }))
        );
        console.log("affectedDuplicatedNodes ", affectedDuplicatedNodes);
      }
      if (affectedDuplicatedNodes.length > 1) {
        emit<TranslationsUpdateHandler>(
          "UPDATE_NODES",
          affectedDuplicatedNodes
        );
      } else {
    ...
    <Container space="medium">
      <div style={{ height: "calc(100% - 48px)", overflow: "auto" }}>
        ...
        <Banner icon={<IconInfo32 />}>
          Checked Items will be synced to Tolgee.
        </Banner>
            ...
            {editedNodes.map((node) => (
              <SelectableItem
                style={{
                  height: "auto",
                  padding:
                    "var(--space-small) var(--space-small) var(--space-small) var(--space-medium);",
                }}
                key={node.id}
                onChange={() => toggleNode(node)}
                value={!!selectedNodes.includes(node)}
